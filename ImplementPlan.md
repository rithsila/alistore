...
- **Objective**: Guest order-status lookup keyed on `display_id` + `track_token`, returning status only.
- **Requirements**: Clone the `backend/src/api/store/orders/[id]/invoice/route.ts` template. zod-validate `id` (display_id) + `token` via middleware before any service call; resolve via `query.graph(entity:"order", filters:{ display_id })` selecting ONLY status-safe fields — `display_id, status, payment_status, fulfillment_status, created_at, metadata.confirmation_status, fulfillments.shipped_at, fulfillments.delivered_at, fulfillments.labels.tracking_number, fulfillments.labels.tracking_url` — **never** phone/address/email. Authorize with `timingSafeEqual` against `order.metadata.track_token`. Rate-limit per-IP **and** per-`display_id` via the existing `overLimit()` helper. Unknown order (404) and bad/expired token (403) return an **identical generic body** so `display_id`s can't be enumerated; errors are `{ error, request_id }`; `Cache-Control: no-store` + `X-Content-Type-Options: nosniff`. Storefront reaches it through the existing `/store/*` publishable-key proxy (`storefront/src/middleware.ts`).
- **Dependencies**: TRACK-01
- **Deliverables**: `backend/src/api/store/orders/track/route.ts`, `backend/src/api/store/orders/track/middlewares.ts` (zod), registration in `backend/src/api/middlewares.ts`
- **Acceptance Criteria**: Valid `id`+`token` → status payload with no PII; wrong/expired token → generic 403; unknown `id` → generic 404 byte-identical to the 403 body; per-IP and per-`display_id` limits enforced; `no-store`/`nosniff` present.
...
- **Requirements**: Mint the `track_token` (TRACK-01) wherever an order is finalized today and return it alongside `invoice_token`: the COD route (`backend/src/api/store/orders/cod/route.ts`) **and** the shared paid-cart finalizer (`finalizePaidCart`, used by the active KHQR provider — currently KHPAY-03) so the link covers whatever KHQR rail is live, not one provider. Carry it to the client through `storefront/src/lib/checkout.ts` (mirror the INTEGRATION-07 invoice-token `?token=` carry) and surface a "Track your order" link on `/order/[id]` (`storefront/src/app/order/[id]/page.tsx`) → `/track?id=<display_id>&token=<track_token>`. The token is only ever forwarded, **never minted client-side**.
- **Dependencies**: TRACK-02, INTEGRATION-04 (COD carry), INTEGRATION-07 (invoice-token carry precedent), KHPAY-03 (`finalizePaidCart`)
- **Deliverables**: edits to `backend/src/api/store/orders/cod/route.ts`, the shared `finalizePaidCart` path, `storefront/src/lib/checkout.ts`, `storefront/src/app/order/[id]/page.tsx`
