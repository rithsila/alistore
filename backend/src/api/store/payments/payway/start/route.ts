/**
 * POST /store/payments/payway/start — PAYWAY-03.
 *
 * Starts an ABA PayWay KHQR payment for a cart and holds stock during the
 * payment window. Mirrors the khqr start route (BACKEND-03) exactly — same
 ...
 * contract — but the QR comes from PayWay's Purchase API (the provider's
 * `initiatePayment` makes that one outbound call), so there is no separate
 * deeplink lookup step.
 *
 * Body: `{ cart_id, currency }` → `{ qr, deeplink, reference, expires_at }`
 * (`reference` is the minted ≤20-char PayWay tran_id). Errors: 409
 * out-of-stock, 502 PayWay down/rejecting.
 *
 ...
 * non-expired PayWay session instead of reserving stock / purchasing again —
 * a customer double-tapping "Pay" can't stack reservations or duplicate
 * PayWay transactions.
 *
 * SECURITY: never log the cart, qr, tran_id, or PayWay bodies (security.md).
 */
...
import { UnsafePayWayUrlError } from "../../../../../modules/aba-payway/lib/client"
import {
  fail,
  getClientIp,
  getRequestId,
...
import type { StartPaywaySchema } from "./middlewares"
...
/** Amount in the selected currency (KHR is whole riel; USD 2dp). */
function resolveAmount(
  currency: StartPaywaySchema["currency"],
  usdTotal: number
): number {
  return currency === "KHR"
    ? usdToKhr(usdTotal)
...
/**
 * Persist the tran_id → cart mapping for the status/pushback endpoints. TTL
...
async function mapTranToCart(
  ...
  const ttl = expiresAt
    ...
    : 1500
  await cache.set(tranCartKey(tranId), cartId, ttl).catch(() => undefined)
}

/**
 * Find an existing PayWay session on the cart that is still usable (has a QR,
 ...
 * PayWay purchases (which would hit PayWay's duplicate-tran_id error anyway).
 */
function findActivePaywaySession(cart: any): ActiveSession | null {
  ...
  for (const session of sessions) {
    ...
    const tranId = data.tran_id as string | undefined
    const expiresAt = data.expires_at as string | undefined
    if (!qr || !tranId || !expiresAt) continue
  ...
  for (const item of cart.items ?? []) {
    ...
    for (const ii of variant.inventory_items ?? []) {
      ...
      reservations.push({
        ...
        description: "KHQR payment hold (PayWay)",
      })
    }
  }
...
export async function POST(
  req: MedusaRequest<StartPaywaySchema>,
  res: MedusaResponse
): Promise<void> {
  ...
  const respond = async (session: ActiveSession): Promise<void> => {
    // Record tran_id → cart so /payway/status and /payway/pushback can
    // resolve this cart later.
    await mapTranToCart(req, session.tranId, cart_id, session.expiresAt)
    res.json({
      qr: session.qr,
      deeplink: session.deeplink,
      reference: session.tranId,
      expires_at: session.expiresAt,
    })
  }

  // Idempotency: reuse an existing, non-expired PayWay session for this cart.
  const existing = findActivePaywaySession(cart)
  if (existing) {
  ...
  // so we can release them if the PayWay purchase fails — otherwise the stock
  ...
  // PayWay payment session — the provider's initiatePayment performs the
  // outbound Purchase call and stores qr/deeplink/tran_id on session.data.
  let session: { data?: Record<string, unknown> }
  try {
    const { result } = await createPaymentSessionsWorkflow(req.scope).run({
      input: {
        payment_collection_id: paymentCollectionId!,
        provider_id: PAYWAY_PROVIDER_ID,
        data: {
          paywayAmount: resolveAmount(currency, usdTotal),
          paywayCurrency: currency,
        },
      },
    })
    ...
    if (err instanceof UnsafePayWayUrlError) {
      logger.error(
        `[payway/start] PAYWAY_BASE_URL failed SSRF validation (request_id=${requestId})`
      )
    } else {
      // PayWay rejected or was unreachable — gateway failure, not our 500.
      logger.warn(
        `[payway/start] PayWay purchase failed (request_id=${requestId})`
      )
    }
  ...
  const tranId = sessionData.tran_id as string | undefined
  const expiresAt = (sessionData.expires_at as string | undefined) ?? null
  if (!qr || !tranId) {
    await releaseReservations()
    logger.error(
      `[payway/start] QR missing from session data (request_id=${requestId})`
    )
  ...
  return respond({
    qr,
    deeplink: (sessionData.deeplink as string | null | undefined) ?? null,
    tranId,
    expiresAt,
  })
}
