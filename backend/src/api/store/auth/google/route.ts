/**
 * GET /store/auth/google — BACKEND-05C (Google OAuth, start).
 *
 * Begins the optional social-login flow by 302-redirecting the browser to
 * Google's OAuth dialog with our `client_id` (GOOGLE_CLIENT_ID), the registered
 * `redirect_uri`, the minimal scopes, and a freshly generated `state`. The
 * code-exchange + identity link happens on the callback (BACKEND-05D).
 *
 * Mirrors the Facebook start route (BACKEND-05) exactly — same state/cookie
 * mechanism, same redirect-uri allowlist, same rate limit — only the provider
 * specifics differ (authorize URL, scopes, env var names, cookie/key names).
 *
 ...
 *
 * SECURITY (security.md "Facebook OAuth" — same rules apply to Google):
 ...
 *    flow to *this* browser — the callback (05D) requires query-state ===
 *    cookie-state AND a live Redis entry, then consumes the entry (single use).
 *    SameSite=Lax (not Strict) is required so the cookie survives Google's
 ...
 *  - Scopes are restricted to `email profile openid` (the minimal set the
 *    built-in Google provider uses).
...
/** Google OAuth 2.0 authorization endpoint (browser-facing authorize URL). */
const GOOGLE_OAUTH_DIALOG_URL = "https://accounts.google.com/o/oauth2/v2/auth"

/** Minimal scopes (security.md): the exact set the built-in Google provider uses. */
const GOOGLE_SCOPES = "email profile openid"

/**
 * The backend path Google redirects back to (BACKEND-05D). The full
 * `redirect_uri` is `<origin>` + this path, where `<origin>` comes from
 * `GOOGLE_OAUTH_REDIRECT_URI` (or its dev default) — never the request host.
 */
const CALLBACK_PATH = "/store/auth/google/callback"

/**
 * Dev-only default callback. In production `GOOGLE_OAUTH_REDIRECT_URI` MUST be
 * set to the exact URL registered in the Google OAuth client; the local default
 * only keeps the flow working against `npx medusa develop` without extra config.
 */
const DEV_DEFAULT_REDIRECT_URI = `http://localhost:9000${CALLBACK_PATH}`

/** State lifetime: long enough to complete the Google dialog, short by design. */
...
export async function GET(
  ...
  if (!clientId) {
    ...
    logger.warn(
      `[auth/google] GOOGLE_CLIENT_ID not configured (request_id=${requestId})`
    )
  ...
  if (!redirectUri) {
    logger.error(
      `[auth/google] GOOGLE_OAUTH_REDIRECT_URI failed validation (request_id=${requestId})`
    )
  ...
  const authorizeUrl = new URL(GOOGLE_OAUTH_DIALOG_URL)
