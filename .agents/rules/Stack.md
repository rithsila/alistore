...
## Storefront file organization (`storefront/src/`)
...
* **Migrations** : Medusa emits timestamped migration files. The agent MAY run `db:generate` and `db:migrate` against the **dev** database (Proxmox). Never run `db:migrate`/`db:reset`/`db:rollback` against **production** (Supabase, post-go-live) from the agent — humans apply those after review + backup.
...
* Run migration (dev only — Proxmox): `npx medusa db:migrate`
