...
* Treat the "Resolved decisions (locked)" section of `ImplementPlan.md` as hard constraints (coral `#C0461F`, English-first v1, env-based exchange rate, Individual KHQR, Proxmox = current dev (agent-migratable) / Supabase = production post-go-live, VAT 10% off-by-default, Medusa `2.15.3` pinned with admin MFA required).

## During task execution

* Use the relevant skill or tool (Medusa MCP, etc.) for conventions when available.
* **The agent MAY run `db:generate` and `db:migrate` against the dev database** (the LAN Proxmox host in `DATABASE_URL`). **Never run `db:migrate`/`db:reset`/`db:rollback` against production** (Supabase, post-go-live) — those are run by a human after review + backup.
